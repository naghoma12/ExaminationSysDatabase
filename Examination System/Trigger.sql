...
CREATE OR ALTER TRIGGER AddAnswerToExam
ON Answer
INSTEAD OF INSERT 
AS
BEGIN 
	IF EXISTS (SELECT 1 FROM Inserted I JOIN Std_Exam ES ON I.NationalId = Es.NationalId AND  I.Exm_Id = ES.ExamId) 
		BEGIN
			PRINT 'You are not assigned to this Exam';
			return;
		END
	INSERT INTO Answer(Ans_Id, Exm_Id , NationalId, Q_Id,  AnswerText)
		SELECT Ans_Id, Exm_Id, NationalId, Q_Id, AnswerText FROM Inserted
END